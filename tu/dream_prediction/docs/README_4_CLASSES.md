...
Tạo ra 2 file:
- `data/dream_data.csv` (tiếng Anh)
- `data/dream_data_vn.csv` (tiếng Việt)
...
Tạo ra:
- `models/mo_hinh_tot_nhat_vn.pkl` - Mô hình đã huấn luyện
- `models/scaler_vn.pkl` - Scaler để chuẩn hóa
- `models/ten_dac_trung_vn.pkl` - Tên các đặc trưng
- Các biểu đồ phân tích trong `reports/figures/` trong `reports/figures/`
...
```
dream_prediction/
├── src/generate_data_4_classes.py    # Tạo dữ liệu 4 loại
├── src/train_model_vn.py             # Huấn luyện mô hình (tiếng Việt)
├── src/predict_vn.py                 # Dự đoán (tiếng Việt)
├── tests/test_4_classes.py             # Test nhanh
├── data/dream_data_vn.csv             # Dữ liệu tiếng Việt
├── models/mo_hinh_tot_nhat_vn.pkl       # Mô hình đã train
├── models/scaler_vn.pkl                 # Scaler
└── models/ten_dac_trung_vn.pkl          # Tên features
```
