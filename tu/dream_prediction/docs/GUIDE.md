...
**Kết quả:**
- Tạo file `data/dream_data.csv` với 1000 mẫu
...
- Tạo visualizations:
  - `reports/figures/model_comparison.png` - So sánh độ chính xác
  - `reports/figures/confusion_matrix.png` - Ma trận nhầm lẫn
  - `reports/figures/feature_importance.png` - Tầm quan trọng đặc trưng (nếu mô hình hỗ trợ)

**Output:**
- `models/mo_hinh_tot_nhat_vn.pkl` - Mô hình tốt nhất
- `models/scaler_vn.pkl` - Scaler để chuẩn hóa dữ liệu
...
**Kết quả:** File `data/dream_data_real.csv` được tạo với ~400 mẫu dữ liệu thật!
...
```
dream_prediction/
├── src/generate_data_4_classes.py          # Tạo dữ liệu giả định
├── src/load_real_data.py         # Tải và xử lý dữ liệu thật
├── auto_download_kaggle.py   # Tự động tải từ Kaggle API
├── src/train_model_vn.py            # Huấn luyện mô hình
├── src/predict_vn.py                # Dự đoán
├── run_all.py                # Master script (chạy tất cả)
├── tests/test_system.py            # Script test toàn diện
├── create_real_data_sample.py # Tạo dữ liệu thật mẫu
├── requirements.txt          # Dependencies
├── GUIDE.md                  # Hướng dẫn này (file duy nhất)
│
├── data/dream_data.csv           # Dữ liệu giả định (sau khi chạy generate_data_4_classes.py)
├── data/dream_data_real.csv      # Dữ liệu thật (sau khi chạy load_real_data.py)
├── sleep_health_lifestyle.csv # Dữ liệu gốc từ Kaggle
│
├── models/mo_hinh_tot_nhat_vn.pkl     # Mô hình tốt nhất (sau khi train)
├── models/scaler_vn.pkl               # Scaler (sau khi train)
│
├── reports/figures/model_comparison.png     # Biểu đồ so sánh (sau khi train)
├── reports/figures/confusion_matrix.png     # Confusion matrix (sau khi train)
├── reports/figures/feature_importance.png   # Feature importance (nếu mô hình hỗ trợ)
│
├── predictions_result.csv   # Kết quả dự đoán hàng loạt
├── data/test_data.csv            # File test mẫu
│
...
### Lỗi: "File not found: models/mo_hinh_tot_nhat_vn.pkl"
...
1. **Model Comparison** (`reports/figures/model_comparison.png`)
   - So sánh độ chính xác và cross-validation scores
   - Bar charts với màu sắc

2. **Confusion Matrix** (`reports/figures/confusion_matrix.png`)
   - Ma trận nhầm lẫn với heatmap
   - Hiển thị số lượng và tỷ lệ

3. **Feature Importance** (`reports/figures/feature_importance.png`)
